...
class LinearRegression:
    def __init__(self,X,Y,features,type='simple',norm_value=1,bias=True):
    ...
    def simple_gradient(self):
        grad = []
        if self.bias:
            partial_b = (self.pred(self.X)-self.Y).sum()
    ...
    def quick_gradient(self,ridge,norm):
            ...
            grad -= np.matmul(self.X.T,self.Y)
            if ridge==True:
                grad += norm * self.w
            if self.bias:
                partial_b = (self.pred(self.X)-self.Y).sum()
                grad = np.insert(grad,0,partial_b)
            return grad
